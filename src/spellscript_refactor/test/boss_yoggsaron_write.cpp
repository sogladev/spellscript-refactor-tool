...
public:
    ...
    struct boss_yoggsaron_saraAI : public ScriptedAI
    {
        ...
        {
            summons.Summon(cr);
            if (cr->GetEntry() == NPC_SANITY_WELL)
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_BRAIN_DAMAGED)
            {
                ...
                if (Creature* cr = ObjectAccessor::GetCreature(*me, m_pInstance->GetGuidData(NPC_THORIM_KEEPER)))
                    cr->CastSpell(cr, SPELL_TITANIC_STORM_PASSIVE, false);
...
const uint32 TABLE_KEEPER_PASSIVE[4] = {SPELL_FREYA_PASSIVE, SPELL_HODIR_PASSIVE, SPELL_MIMIRON_PASSIVE, SPELL_THORIM_PASSIVE};
        ...
        boss_yoggsaron_keeperAI(Creature* pCreature) : NullCreatureAI(pCreature)
        {
            ...
            switch (me->GetEntry())
            {
                case NPC_FREYA_KEEPER:
                    _keeperId = KEEPER_FREYA;
                    break;
                case NPC_HODIR_KEEPER:
                    _keeperId = KEEPER_HODIR;
                    break;
                case NPC_MIMIRON_KEEPER:
                    _keeperId = KEEPER_MIMIRON;
                    break;
                case NPC_THORIM_KEEPER:
                    _keeperId = KEEPER_THORIM;
                    break;
                default:
                    return;
            }
            me->RemoveAura(TABLE_KEEPER_PASSIVE[_keeperId]);

            scheduler.Schedule(1s, [this](TaskContext context)
            {
                if (me->GetInstanceScript())
                {
                    if (me->GetInstanceScript()->GetData(TYPE_YOGGSARON) == IN_PROGRESS && !_isEncounterInProgress)
                    {
                        _isEncounterInProgress = true;
                        if (_keeperId == KEEPER_FREYA)
                            me->CastSpell(me, SPELL_CONJURE_SANITY_WELL, false);
                        if (_keeperId == KEEPER_MIMIRON)
                        {
                        ...
                        if (!me->HasAura(TABLE_KEEPER_PASSIVE[_keeperId]))
                            me->AddAura(TABLE_KEEPER_PASSIVE[_keeperId], me);
                    }
                    else if (me->GetInstanceScript()->GetData(TYPE_YOGGSARON) != IN_PROGRESS && _isEncounterInProgress)
                    {
...
class spell_yogg_saron_destabilization_matrix_aura : public AuraScript
{
    PrepareAuraScript(spell_yogg_saron_destabilization_matrix_aura);
...
class spell_yogg_saron_lunatic_gaze_aura : public AuraScript
{
    PrepareAuraScript(spell_yogg_saron_lunatic_gaze_aura);
    ...
    void OnEffect(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetCaster())
            if (InstanceScript* instance = caster->GetInstanceScript())
                ...
                {
                    sara->SummonCreature(NPC_SANITY_WELL, 2042.56f, -40.3667f, 329.274f, 0.0f);
                    sara->SummonCreature(NPC_SANITY_WELL, 1975.89f, 40.0216f, 331.1f, 0.0f);
                    sara->SummonCreature(NPC_SANITY_WELL, 1987.12f, -91.2702f, 330.186f, 0.0f);
                    sara->SummonCreature(NPC_SANITY_WELL, 1900.48f, -51.2386f, 332.13f, 0.0f);
                    sara->SummonCreature(NPC_SANITY_WELL, 1899.94f, 0.330621f, 332.296f, 0.0f);
                }
    }
...
void AddSC_boss_yoggsaron()
{
    ...
    RegisterSpellScript(spell_yogg_saron_brain_link_aura);
    RegisterSpellScript(spell_yogg_saron_shadow_beacon_aura);
    RegisterSpellScript(spell_yogg_saron_destabilization_matrix_aura);
    RegisterSpellScript(spell_yogg_saron_titanic_storm_aura);
    RegisterSpellScript(spell_yogg_saron_lunatic_gaze_aura);
