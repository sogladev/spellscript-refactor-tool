...
def generate_sql_update_script_name(script_name_original: str, script_name: str) -> str:
    ...
    if len(spell_ids) == 0:
        ...
        sql_update_script_name = """UPDATE `spell_script_names` SET `ScriptName`='{}' WHERE `spell_id`={};\n""".format(script_name, spell_id)
        return sql_update_script_name
    else:
        spell_ids_format = format_spell_ids(spell_ids)
        sql_update_script_name = """UPDATE `spell_script_names` SET `ScriptName`='{}' WHERE `spell_id` IN {};\n""".format(script_name, spell_ids_format)
        return sql_update_script_name
